...
module.exports = function (
  ...
  async function pollStatus(requestId, to, userLanguage) {
    ...
    if (activePolls.has(requestId)) {
      logger.warn(
        `Polling already active for request ID: ${requestId}. Skipping new poll start.`
      );
      return;
    }
    ...
    const intervalId = setInterval(async () => {
      ...
      if (attempts > MAX_POLLING_ATTEMPTS) {
        logger.warn(
          `Max polling attempts reached for request ID: ${requestId}`
        );
        await sendMessage(
          to,
          userLanguage === "malayalam"
      ...
      try {
        ...
        if (TERMINAL_STATUSES.includes(data.status?.toLowerCase())) {
          ...
          switch (data.status.toLowerCase()) {
            case "submitted":
              message =
                userLanguage === "malayalam"
                  ? `✅ (${requestId}) നിങ്ങളുടെ ഡോക്യുമെന്റുകൾ വിജയകരമായി ലഭിച്ചു! അഭ്യർത്ഥനയുടെ നില അറിയാൻ, ദയവായി '/service' ഉപയോഗിച്ച് പരിശോധിക്കുക.`
                  : `✅ (${requestId}) Your documents have been received successfully! and for knowing the status of your application, please check the status using /service.`;
              break;
            case "rejected":
            case "failed":
              message =
        ...
        logger.error(
          `Error polling status for ${requestId}:`,
  ...
  async function cancelAndGoToMainMenu(user, From) {
    ...
    const optionModule = require("./optionModule")(
      sendMessage,
      module.exports,
      null,
      logger,
      AXIOS_TIMEOUT
    ); // Re-require partially to avoid circular dep issues if structured differently
  ...
  return {
    process: async (Body, user, From) => {
      ...
      if (lower === "/cancel") {
        ...
        try {
          logger.info(
            `User ${user.phoneNumber} attempting to cancel request ${last.serviceRequestId}`
          );
          await axios.post(
          ...
          user.applications = apps.filter(
            (a) => a.serviceRequestId !== last.serviceRequestId
          );
          ...
          logger.error(
            `Error cancelling request ${last.serviceRequestId}:`,
            err.response?.data || err.message
          );
          await sendMessage(
            From,
      ...
      if (lower === "0") {
        logger.info(
          `User ${user.phoneNumber} cancelled application flow at state: ${user.applyState}`
        );
        await sendMessage(
          From,
          user.language === "malayalam"
      ...
      if (lower === "back") {
        ...
        switch (user.applyState) {
          ...
          case "document":
            ...
            const subs = await CentreModel.distinct("subdistrict", {
              district: user.applyDataTemp.district,
            });
            if (!subs.length) {
              // Should not happen if they got here, but check anyway
              message =
                `No subdistricts found for ${user.applyDataTemp.district}. Returning to district selection.\n*Select district:* (0️⃣ Cancel)\n` +
                DISTRICTS.map((d, i) => `${i + 1}. ${d}`).join("\n");
              user.applyState = "district";
            } else {
              message =
          ...
          default: // 'back' from initial state or unexpected state
            logger.info(
              `User ${user.phoneNumber} used 'back' from state: ${user.applyState}. Returning to menu.`
            );
            await sendMessage(
              From,
              user.language === "malayalam"
                ? "പ്രധാന മെനുവിലേക്ക് മടങ്ങുന്നു."
      ...
      if (user.applyState === "district") {
        ...
        await user.save();
        logger.info(
          `User ${user.phoneNumber} selected district: ${user.applyDataTemp.district}`
        );
        ...
        try {
          ...
          if (!subs.length) {
            ...
            await user.save();
            logger.warn(
              `No subdistricts found for district: ${user.applyDataTemp.district}`
            );
            const msg =
              user.language === "malayalam"
          ...
          logger.error(
            `DB Error fetching subdistricts for ${user.applyDataTemp.district}:`,
            dbError
          );
          await sendMessage(
            From,
            "Error fetching subdistricts. Please try again or type 0 to cancel."
          );
      ...
      if (user.applyState === "subdistrict") {
        ...
        await user.save();
        logger.info(
          `User ${user.phoneNumber} selected subdistrict: ${user.applyDataTemp.subdistrict}`
        );
        ...
        try {
          ...
          if (!services.length) {
            ...
            await sendMessage(
              From,
              "Sorry, no document services are configured currently. Please contact support."
            );
            return cancelAndGoToMainMenu(user, From);
          }
          const prompt =
          ...
          await sendMessage(
            From,
            "Error fetching document types. Please try again or type 0 to cancel."
          );
      ...
      if (user.applyState === "document") {
        ...
        await user.save();
        logger.info(
          `User ${user.phoneNumber} selected document: ${svc.name} (${svc.key})`
        );
        ...
        try {
          ...
          if (!centres.length) {
            ...
            await user.save();
            logger.warn(
              `No centres found for service ${svc.key} in ${user.applyDataTemp.subdistrict}, ${user.applyDataTemp.district}`
            );
            const msg =
              user.language === "malayalam"
          ...
          logger.error(
            `DB Error fetching centres for ${svc.key} in ${user.applyDataTemp.subdistrict}:`,
            dbError
          );
          await sendMessage(
            From,
            "Error fetching service centres. Please try again or type 0 to cancel."
          );
      ...
      if (user.applyState === "centre") {
        ...
        logger.info(
          `User ${user.phoneNumber} selected centre: ${chosen.centreName} (${chosen.centreId})`
        );
        ...
        try {
          ...
          logger.info(
            `Sending service request payload: ${JSON.stringify(payload)}`
          );
          ...
          logger.error(
            "API error creating service request:",
            err.response?.data || err.message
          );
          const errorMsg =
            user.language === "malayalam"
              ? `അഭ്യർത്ഥന സൃഷ്ടിക്കുന്നതിൽ പിശക് (${
                  err.response?.status || "Network Error"
                }). ദയവായി വീണ്ടും ശ്രമിക്കുക അല്ലെങ്കിൽ റദ്ദാക്കാൻ 0️⃣ നൽകുക.`
              : `Error creating request (${
                  err.response?.status || "Network Error"
                }). Please try again or enter 0️⃣ to cancel.`;
      ...
      logger.warn(
        `User ${user.phoneNumber} reached unexpected applyState: ${user.applyState} with input: ${Body}`
      );
      const unexpectedStateMsg =
        user.language === "malayalam"
